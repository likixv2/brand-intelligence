...
function parseItems(xml, account) {
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    const tweetUrl = link.replace(/https?:\/\/[^/]+\//, 'https://x.com/');
    if (title.startsWith('RT by') || title.startsWith('R to')) continue;
    const text = title.replace(/^@\w+: /, '').trim();
    if (!text || text.length < 20) continue;
    const date = new Date(pubDate);
    const hoursAgo = (Date.now() - date.getTime()) / 3600000;
    const score = Math.max(0, 100 - hoursAgo * 2) + Math.min(30, text.length / 5);
    items.push({ handle: '@' + account, text, url: tweetUrl, pubDate, hoursAgo: Math.round(hoursAgo), score: Math.round(score) });
  }
...
function classifyTweet(text) {
  ...
  if (t.includes('wrong') || t.includes('unpopular') || t.includes('hot take') || t.includes('disagree') || t.includes('actually')) return 'debate';
  return 'viral';
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (allTweets.length === 0) {
      return res.status(200).json({ tweets: [], error: 'Nitter may be temporarily down. Try refreshing in a few minutes.' });
    }
